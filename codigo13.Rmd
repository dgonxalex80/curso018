...
```{r}
t1 <- table(clientes$categoria_preferida)
cols <- c("#2C5697", "#3B9AC4", "#245B89", "#2C78A8", "#B8DDEB" )
pie(t1, col = cols)
```
...
fig=ggplot(data=ventas,  aes(x=Edad , y=Ventas_Netas))+
  ...
  stat_smooth(method = "loess" , formula =y ~ x, color = "#245B89")
fig
```
...
fig2 = ggplot(data=ventas, aes(x=Ventas_Netas, y=Tipo_Cliente)) +
  geom_boxplot(fill = "#2C5697", color = "#2C78A8")+
...
fig3=ggplot(clientes, aes(edad_cliente)) +
       geom_histogram(bins = 7,fill="#3B9AC4", color="#F7F7F7", alpha=0.9)+
       theme_minimal() +
...
fig=ggplot(data = ventas, aes(x = Tipo_Cliente, fill = Metodo_Pago)) + 
  ...
  coord_polar() +
  scale_fill_manual(values = c("#2C5697", "#3B9AC4", "#245B89", "#2C78A8", "#B8DDEB" ))
fig
```
```{r}
# Plot
ggplot(ventas, aes(x = Tipo_Cliente, y = Ventas_Netas, fill = Tipo_Cliente)) +
  ...
  scale_fill_manual(values = c("#2C5697", "#3B9AC4")) +
  theme_minimal() +
...
if (load_pkg("ggbeeswarm")) {
  ggplot(ventas, aes(x = Tipo_Cliente, y = Ventas_Netas, color = Tipo_Cliente)) +
    geom_beeswarm(cex = 2) +
    scale_color_manual(values = c("#2C5697", "#245B89"))
}
```
...
if (load_pkg("ggridges")) {
  ggplot(df, aes(x = precio, y = tipo, fill = tipo, color = tipo)) +
    ...
    scale_fill_manual(values =c("#2C5697", "#3B9AC4", "#245B89", "#2C78A8", "#B8DDEB" )) +
    scale_color_manual(values = c("#B8DDEB", "#2C78A8", "#245B89", "#3B9AC4","#2C5697"))
}
```
...
polygon(deny, col = adjustcolor("#245B89", alpha.f = 0.6))
```
...
p <- ggplot(resumen, aes(x = mes, y = Ventas_Netas, color = Tipo_Cliente, group = Tipo_Cliente)) +
  ...
  geom_point(size = 2) +
  scale_color_manual(values = c("#2C5697", "#245B89"))  +   # paleta institucional (5 colores)
  labs(
...
p <- datos %>%
  ...
  scale_color_manual(values = "#3B9AC4") +
...
if (ok_rank) {
  ...
  ranked_by_year %>%
    ...
    geom_rect(alpha = .7) +
    aes(fill = continent) +
    scale_fill_manual(values = c("#2C5697", "#245B89", "#2C78A8", "#8FC7DE")) +
    scale_x_continuous(
      limits = c(-800, 1400),
...
if (is.null(cali)) { # valida que se haya cargado
  ...
  plot(sf::st_geometry(cali), col = "#B8DDEB" , border = "#2C5697", # geometría y colores
...
<html>
<button id="dl-rmd" style="background:#73B8D4;color:#fff;padding:7px 20px;border:0;border-radius:6px;cursor:pointer">
...
<html>
<button id="dl-practica" style="background:#73B8D4;color:#fff;padding:7px 20px;border:0;border-radius:6px;cursor:pointer">
...
<html>
<button id="dl-beer" style="background:#73B8D4;color:#fff;padding:7px 20px;border:0;border-radius:6px;cursor:pointer">
